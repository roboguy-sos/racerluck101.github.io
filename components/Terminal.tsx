...
import { profile, socials, skillGroups, certifications } from "@/lib/data";
...
function runCommand(cmd: string): string[] {
    ...
    switch (c) {
        ...
        case "ls":
            return [
                "about  skills  certifications  projects  contact  resume.pdf",
            ];
        case "sudo":
        case "sudo su":
...
export default function Terminal() {
    ...
    const submit = (raw: string) => {
        ...
        if (cmd.toLowerCase() === "clear") {
            ...
            const out = runCommand(cmd).map((text) => ({
    ...
    const onKeyDown = (e: React.KeyboardEvent<HTMLInputElement>) => {
        if (e.key === "Enter") {
            ...
            const idx =
                hIdx === -1 ? history.length - 1 : Math.max(0, hIdx - 1);
            setHIdx(idx);
                        ...
                        >
                            {lines.map((line, i) =>
                                line.type === "input" ? (
                                    ...
                                    <p
                                        key={i}
                                        className="whitespace-pre-wrap break-words text-white/60"
                                    >
                                        {line.text}
                                    </p>
                                ),
